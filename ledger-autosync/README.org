...
*** Installation
1. ~pip3 install https://github.com/egh/ledger-autosync/archive/refs/heads/master.tar.gz~
2. Create ledger-autosync .config folder: ~mkdir $HOME/.config/ledger-autosync~
3. Symlink the plugins: ~ln -s $HOME/git/dotfiles/ledger-autosync/plugins/ $HOME/.config/ledger-autosync/~
*** Workflow
